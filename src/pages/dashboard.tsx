...
import { HealthcareStaffListPage as ShowCaseList } from 'pages/healthcare-staffs';

function DashboardPage() {
  const { session } = useSession();

  const healthcareStaffStat = useSWR<PaginatedInterface<HealthcareStaffInterface>>(
    () => `/healthcare-staffs`,
    () => getHealthcareStaffs({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: healthcareStaffStat?.data?.totalCount,
      isLoading: healthcareStaffStat.isLoading,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Healthcare Staffs
                </Text>
                <NextLink href={`healthcare-staffs`} passHref legacyBehavior>
                  <Button

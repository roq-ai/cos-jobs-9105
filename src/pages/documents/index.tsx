...
export function DocumentListPage(props: DocumentListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getDocuments({
        relations: ['healthcare_staff'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['document_name.contains', 'document_type.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('healthcare_staff', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'healthcare_staff',
          header: 'Healthcare Staff',
          accessorKey: 'healthcare_staff',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/healthcare-staffs/view/${record.healthcare_staff?.id}`}
            >
              {record.healthcare_staff?.first_name}
            </Link>
          ),
        }
      : null,
